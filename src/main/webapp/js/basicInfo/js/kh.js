layui.use(['table','layer','form'], function(){
    ...
    table.render({
        elem: '#supplier'
        ,url:path+'supplier/querySupplier'
            ...
            case 'addkh':
                layer.open({
                    ...
                    yes:function(index, layero){
                        ...
                        basesubmit(path+'supplier/addSupplier',body.find("#addsupplier").serialize());
    ...
    //查询
    $("#searchkh_btn").on("click", function () {
        var name = $("#name").val();
    ...
    table.on('tool(supplier)', function(obj){
        ...
        if(obj.event === 'delkh'){
            layer.confirm('真的删除行么', function(index){
                basesubmit(path+'supplier/delSupplier', { uuid: data.uuid });
                layer.close(index);
                table.reload("supplierTable",{});
            });
            ...
            layer.open({
                ...
                yes:function(index,b){
                    var body = layer.getChildFrame('body', index);
                    basesubmit(path+'supplier/editSupplier',body.find("#addsupplier").serialize());
                    layer.close(index); //再执行关闭，强制关闭弹窗
                    table.reload("supplierTable",{});
                }
            });
